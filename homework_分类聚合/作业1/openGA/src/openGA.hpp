...
//没见过的代码以及头文件都查询以后写在旁边
#pragma once
#include <vector>
#include <random>
#include <chrono>//用于计时的库文件
#include <thread>
#include <ctime>
#include <string>
#include <iostream>
...
enum class GA_MODE//定义算法运行模式
{
    SOGA,//我们今天只看这种最简单的方式--单目标遗传算法
    IGA,
    NSGA_III
};
...
public:
    ...
    {
        /*
         * 流程：solve 第一步，造第0代种群，算一遍适应度
         * 成员变量：generation_step 变成 0，last_generation 更新
         * 依赖：init_population（会调用户的 init_genes/eval_solution）
         */
        check_settings();
        // shrink_scale=1.0;
    ...
    {
        /*
         * 流程：一代循环——保留、交叉变异、再选择、更新最优
         * 局部变量：new_generation 新种群
         * 依赖：transfer, crossover_and_mutation, select_population 等
         */
        Chronometer timer;
        timer.tic();
        generation_step++;
    ...
    {
        /*
         * 流程：先 init 第0代，再 while 循环 next_generation 直到停
         * 局部变量：stop 停止原因
         * 依赖：solve_init, solve_next_generation
         */
...
protected:
    ...
    ****************************************************/
    ...
    {
        /*
         * 流程：随机生成 population 个个体
         * 依赖：用户的 init_genes 和 eval_solution
         */
